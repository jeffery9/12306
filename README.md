...
>
> 本项目以 [**CQRS（读写分离）**](./docs/12306%20高并发票务系统技术方案.md) 与 **EDA（事件驱动）** 为核心架构思想，在 **AI Agent（Gemini CLI 极致去幻觉协议 + ChatGPT 首席架构师）** 的深度协作下，使用纯粹而精纯的 **Python 3.9 + FastAPI + SQLAlchemy + Redis Lua + Kafka** 核心生态，实现了针对“一车多站、区间座位复用、防死锁、最终一致性”这一 L3/L4 级别核心难题的工业级可运行垂直切片 MVP。
>
